<template>
  <div id="homeCollection">
    <div class="home_header">
      <van-nav-bar title="实时收款申请" left-arrow @click-left="$router.back()" />
    </div>
    <div>
      <van-form ref="form">
        <van-field
...
export default {
  ...
  methods: {
    ...
    selectCard(val) {
      this.select1 = false;
      this.form.payBankName = val.name;
    },
    // 获取银行卡
    getBankList() {
      this.$post({
    ...
    async submitApproval() {
      ...
      if (this.form.payBankName === "") {
        return this.$toast("请输入收款人开户行");
      }
      if (this.form.payBankNumber === "") {
      ...
      if (this.form.reason === "") {
        return this.$toast("请输入收取理由");
      }
      const res = await this.$post({
        url: "/erp/vphonor/auditRule/saveAuditRecordReceiveSingle",
        data: {
          id: 1,
          payName: this.form.payName,
          payeeType: Number(this.form.payeeType),
          payBankName: this.form.payBankName,
          payBankNumber: this.form.payBankNumber,
